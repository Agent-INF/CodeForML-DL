function ret = a3(wd_coefficient, n_hid, n_iters, learning_rate, momentum_multiplier, do_early_stopping, mini_batch_size)
  ...
  for data_i = 1:3,
    ...
    ret.loss(data_i) = loss(model, data, wd_coefficient);
    ret.error(data_i) = classification_performance(model, data);
    fprintf('\nThe loss on the %s data is %f\n', data_name, ret.loss(data_i));
    if wd_coefficient~=0,
      ret.closs(data_i) = loss(model, data, 0);
      fprintf('The classification loss (i.e. without weight decay) on the %s data is %f\n', data_name, ret.closs(data_i));
    end
    fprintf('The classification error rate on the %s data is %f\n', data_name, ret.error(data_i));
  end
end
...
%  grad_class_input = - data.targets .* (1 - class_prob) / size(data.inputs, 2);         % classes * cases
  grad_class_input = (class_prob - data.targets) / size(data.inputs, 2);
  ret.hid_to_class = grad_class_input * hid_output' + wd_coefficient * model.hid_to_class;  
  grad_hid_output = model.hid_to_class' * grad_class_input;                % hid * cases
  grad_hid_input = grad_hid_output .* (hid_output .* (1 - hid_output)); % hid * cases
  ret.input_to_hid = grad_hid_input * data.inputs' + wd_coefficient * model.input_to_hid;   %hid * cases
end
